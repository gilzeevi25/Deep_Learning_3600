...
from project.hyperparams import PART4_CUSTOM_DATA_URL as CUSTOM_DATA_URL
...
def load_WGAN(ds_gwb,name,device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'),n=15):
    hp = wgan_hyperparams()
    checkpoint_file_final = f'checkpoints/{name}.pt'
    if name.lower().find('sn') < 0:
        if name.lower().find('ncritic') > -1:
            hp['n_critic'] =int(name.split('_')[-1])
        print(f"*** Chosen HyperParameters for WGAN: *** \n\n{hp}\n\n")
#         print('*** Sampled Images Generated from WGAN: ***')
    else:
        if name.lower().find('ncritic') > -1:
            hp['n_critic'] =int(name.split('_')[-1])
        print(f"*** Chosen HyperParameters for WGAN_SN: *** \n\n{hp}\n\n")
#         print('*** Sampled Images Generated from WGAN_SN: ***')
#     gen = torch.load(checkpoint_file_final, map_location=device)
#     samples = gen.sample(n, with_grad=False).cpu()
#     fig, _ = plot.tensors_as_images(samples, nrows=3, figsize=(6,6))
...
def train_batch(
    ...
    if wgan == False:
        ...
        dsc_loss = dsc_loss_fn(y_pred, gen_pred, wgan, hp)
        dsc_loss.backward()
        dsc_optimizer.step()
        ...
        #number of updates on discriminator per update of generator
        dsc_batch_losses = []
        for d_iter in range(hp['n_critic']):
            ...
            dsc_loss = dsc_loss_fn(y_pred, gen_pred, wgan, hp)
            dsc_loss.backward()
            dsc_batch_losses.append(dsc_loss.item())
            dsc_optimizer.step()
    ...
    gen_loss = gen_loss_fn(gen_data, wgan, hp)
    gen_loss.backward()
    gen_optimizer.step()
