...
class KNNClassifier(object):
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            ...
            # ====== YOUR CODE: ======
            values_top_k, indices_top_k = torch.topk(dist_matrix[:,i], self.k, largest=False, out=None) #return smallest = closest ranking
            y_pred[i], index_most_frequent = torch.mode( self.y_train[indices_top_k] ) #get label of most ranked closest k
            # ========================
...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    dists = torch.sqrt( a_sq.reshape(-1,1) + b_sq.T - 2 * torch.matmul(x1, x2.T))
    # ========================
