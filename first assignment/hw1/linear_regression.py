...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # ====== YOUR CODE: ======
        I = np.eye((X.shape[1]))
        l2_reg =  self.reg_lambda * I* X.shape[0]
        l2_reg[0,0] = 1
        w_opt = (np.linalg.inv(X.T @ X + l2_reg) @ X.T @ y)
        # ========================
...
class BiasTrickTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: np.ndarray):
        ...
        xb = None
        # ====== YOUR CODE: ======
        xb = np.hstack([np.ones(X.shape[0]).reshape(-1, 1), X])
        # ========================
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    # ====== YOUR CODE: ======
    tar_f = df.corr(method='pearson').loc[target_feature]
    tar_f = tar_f[tar_f < 1].abs().sort_values(ascending=False)
    top_n_features, top_n_corr =  tar_f[0:n].index.tolist(), tar_f[0:n].values.tolist()
    # ========================
...
def mse_score(y: np.ndarray, y_pred: np.ndarray):
    ...
    # ====== YOUR CODE: ======
    # mse = np.sum((y - y_pred) ** 2) / (2 * y.shape[0])
...
def cv_best_hyperparams(
    ...
    # ====== YOUR CODE: ======
    best_params = None
    kfold_cv = sklearn.model_selection.KFold(k_folds,shuffle=True)
    # print(model.get_params().keys())
    tuned_parameters = [
        {'bostonfeaturestransformer__degree': degree_range, 'linearregressor__reg_lambda': lambda_range}]
    mymodel = sklearn.model_selection.GridSearchCV(model,tuned_parameters,cv=kfold_cv,scoring='neg_mean_squared_error').fit(X,y)
