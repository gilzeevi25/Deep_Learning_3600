    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            # test:
            test_result = self.train_epoch(dl_test, **kw)
            test_loss.extend(test_result.losses)
            test_acc.append(test_result.accuracy)
            curr_loss = torch.tensor(test_result.losses).mean().item()
...
class RNNTrainer(Trainer):
    ...
    def train_epoch(self, dl_train: DataLoader, **kw):
        ...
        # ====== YOUR CODE: ======
        self.hidden_state = None # check if necessary
        # ========================
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        h = None if self.hidden_state is None else self.hidden_state.detach()
        # Forward pass
        layer_output,self.hidden_state = self.model(x, h)
        out = layer_output.flatten(start_dim=0, end_dim=1)
        y_out = y.flatten(start_dim=0, end_dim=1)
        # Backward pass
        loss = self.loss_fn(out,y_out)
        self.optimizer.zero_grad()
        loss.backward()
        # Optimizer step
        self.optimizer.step()
        # Calculate accuracy
        num_correct = (out.argmax(axis=1) == y_out).sum()
        # ========================
