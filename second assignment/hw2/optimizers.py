...
class VanillaSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            #theta <- theta - LR * gradient(theta) * loss
            dp+= self.reg* p
...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        # ====== YOUR CODE: ======
        self.vt = [torch.zeros(1)] * len(self.params)
        self.idx = range(len(self.params))
        # ========================
    ...
    def step(self):
        ...
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            self.vt[self.idx[j]] = self.momentum *self.vt[self.idx[j]] - self.learn_rate*dp
            p += self.vt[self.idx[j]]
            j+=1
            # ========================
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        # TODO: Add your own initializations as needed.
        # ====== YOUR CODE: ======
        self.rt = [torch.zeros(1)] * len(self.params)
        self.idx = range(len(self.params))
        # ========================
    ...
    def step(self):
        ...
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            self.rt[self.idx[j]] = self.decay *self.rt[self.idx[j]] +(1 - self.decay)*(dp**2)
            p -=(self.learn_rate / torch.sqrt(self.rt[self.idx[j]]+self.eps))*dp
            j+=1
            # ========================
