...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # first input layer - # calculate padding assuming odd kernel size
        main_layers.append(nn.Conv2d(in_channels, channels[0], kernel_size=kernel_sizes[0], padding=(int((kernel_sizes[0] - 1) / 2))))
        if dropout != 0.0: main_layers.append(nn.Dropout2d(dropout))
        if batchnorm == True: main_layers.append(nn.BatchNorm2d(channels[0]))
