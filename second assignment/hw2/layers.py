...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        dx = dout.clone()
        # keep dx when x>0
        dx = torch.where(x <= 0, self.alpha * dx, dx)
        # ========================
...
class ReLU(LeakyReLU):
    ...
    def __init__(self):
        # ====== YOUR CODE: ======
        super().__init__()
        self.alpha = 0 #special case of LeakyReLU with 0 if x<=0
        # ========================
...
class Linear(Layer):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        self.b = torch.normal( mean=0, std=1, size=(1, self.out_features) )
        # ========================
    ...
    def backward(self, dout):
        ...
        # Z = X*W.T + b
        # --> gradient calculation https://web.eecs.umich.edu/~justincj/teaching/eecs442/notes/linear-backprop.html
        # Z = out --> dout = dL/dz = gradient of loss with respect of forward function
        # accumulate gradients per batch
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # x_y = get the index y of x. index_select returns x.shape tensor, the diagonal is the class score of the truth label
        x_y = torch.index_select(x, 1, y).diagonal()
        x_y = x_y.reshape( len(x_y), 1 )

        loss = torch.log( torch.exp(x).sum( dim=1 ) ) - x_y
        loss = loss.mean() #avg cross entropy
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        # Loss = -x_y + log( sum_k(e^[x_y]) )
        # Z = out --> dout = dL/dz = gradient of loss with respect of forward function = dL/dz * dz/dx
        # --> gradient calculation http://machinelearningmechanic.com/deep_learning/2019/09/04/cross-entropy-loss-derivative.html

        x_mask_y = torch.zeros(x.shape, dtype=torch.int32)
        mask_tuple = (torch.arange(N), y)
        x_mask_y[ mask_tuple ] = torch.tensor(1, dtype=torch.int32)

        exp_x = torch.exp(x)
        soft_max_classif = ( exp_x.T / exp_x.sum(dim=1) ).T

        dx = ( soft_max_classif - x_mask_y ) * dout
        dx = dx / N
...
class Sequential(Layer):
    ...
    def forward(self, x, **kw):
        ...
        for i in range( len(self.layers) ):
            out = self.layers[i].forward( out, **kw )
        # ========================
    ...
    def backward(self, dout):
        ...
        for i in range( len(self.layers) -1, -1, -1 ):
            din = self.layers[i].backward( din )
        # ========================
    ...
    def params(self):
        ...
        # ====== YOUR CODE: ======
        for i in range( len(self.layers) ):
            params[ len(params): ] = self.layers[i].params()
        # ========================
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        out = x
        if not self.training_mode: #dont use dropout in test
            return out
        out = x
        probs = torch.ones(x.shape)*self.p
        drops = torch.bernoulli(probs) * (1/(1-self.p)) # dropout bernoulli probs multiplied by activation scaling
        out = x * drops
