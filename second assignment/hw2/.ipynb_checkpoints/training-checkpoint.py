...
class Trainer(abc.ABC):
    ...
    def fit(
        ...
        for epoch in range(num_epochs):
            ...
            #    argument.
            # ====== YOUR CODE: ======
            # EpochResult(losses=losses, accuracy=accuracy) -> loss,accuracy
            # Saving Losses , Accuracies:
            actual_num_epochs += 1
            # train:
            tr_ls, tr_acc = self.train_epoch(dl_train, **kw)
            train_loss.extend(tr_ls)
            train_acc.append(tr_acc)

            # test:
            act_ls, act_acc = self.train_epoch(dl_test, **kw)
            test_loss.extend(act_ls)
            test_acc.append(act_acc)
            curr_loss = torch.tensor(act_ls).mean().item()

            #several early stopping conditions:

            if early_stopping is not None:
                if p_loss > 0 and curr_loss >= p_loss:
                    print("ENTERED epochs_without_improvement")
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # Backward pass
        loss = self.loss_fn(z, y)  # Single batch (train_batch/test_batch)
        self.optimizer.zero_grad()
        self.model.backward(self.loss_fn.backward())
        # Optimizer step
        self.optimizer.step()
        # Calculate accuracy
        num_correct = (z.argmax(axis=1) == y).sum().item()
        # ========================
...
class TorchTrainer(Trainer):
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        with torch.no_grad():
            ...
            loss = self.loss_fn(z, y).item()  # Single batch (train_batch/test_batch)
            # Calculate accuracy
            num_correct = (z.argmax(axis=1) == y).sum().item()
